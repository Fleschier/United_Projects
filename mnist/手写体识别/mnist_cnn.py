...
def interence(input_tensor,train,regularizer):
    ...
    # 接下来是全连接层，需要将pool2转换为一维向量，作为后面的输入, [bz, 7, 7, 64]
    ...
    with tf.variable_scope('layer5-fc1'):
        ...
        try:
            # 只有全连接层的权重需要加入正则化, regularizer(fc1_w)放入loss集合中
            if regularizer != None:
    ...
    with tf.variable_scope('layer6-fc2'):
        ...
        try:
            if regularizer != None:                 # regularizer(fc2_w)放入loss集合中
