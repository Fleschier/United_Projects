...
def infer(reshape_xs):
    # print(sess.run([y, r], feed_dict={x:reshape_xs}))
    print(sess.run(r, feed_dict={x:reshape_xs})[0])
        # tf.global_variables_initializer().run()
        # for i in range(TRAIN_STEP):
            # 由于神经网络的输入大小为[BATCH_SIZE,IMAGE_SIZE,IMAGE_SIZE,CHANNEL]，因此需要reshape输入。
            # numpy.ndarray, 
            # shape, xs: (bz, 784)   ys:(bz, 10) 
            # type,  xs: float32    ys: float64
            ...
            # if i % 30000 == 0:
                # saver.save(sess,os.path.join(MODEL_PATH,MODEL_NAME+"_step_"+str(i)),global_step=global_step)
